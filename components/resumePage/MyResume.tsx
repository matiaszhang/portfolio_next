...
export default function MyResume() {
  const { data, error } = useQuery<ExperienceQuery>(resumeOperations.Queries.getExperience);
      ...
      <li className="py-8 px-12">
        ...
        {filteredData === undefined ? (
          ...
          filteredData[0].map((r, idx) => (
            <ResumeItem key={r.id} resume={r} border={idx !== filteredData[0].length - 1} />
          ))
        )}
      </li>
      ...
      <li className="py-8 px-12 relative vCustomLine sm:before:block before:hidden before:left-0">
        ...
        {filteredData === undefined ? (
          ...
          filteredData[1].map((r, idx) => (
            <ResumeItem key={r.id} resume={r} border={idx !== filteredData[1].length - 1} />
          ))
        )}
      </li>
    </ul>
