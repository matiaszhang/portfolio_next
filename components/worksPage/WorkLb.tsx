...
export default function WorkLb({ workId, reactiveVar }: Props) {
  const { data: workData } = useQuery<SingleWorkQuery, SingleWorkVariables>(workOperations.Queries.getSingleWork, {
        ...
        {workData === undefined ? (
          ...
          <div className="w-full h-full max-h-full overflow-y-scroll myScroll">
            <div className="w-full relative h-[45rem]">
              ...
              />
              <Image src={workData.work.images[0].url} alt="project description" layout="fill" objectFit="cover" />
            </div>
            <section className="py-14 px-14">
              ...
              <p className="text-xl tracking-wide text-gray-400 capitalize">project</p>

              <ul className="grid grid-cols-1 mt-10 sm:grid-cols-2 gap-y-2">
                <MyInfo field="created by" value={workData.work.ownerName} />
                <MyInfo field="date" value={new Date(workData.work.date).toLocaleDateString()} />
              ...
              <article className="my-10 prose max-w-none">
                <ReactMarkdown remarkPlugins={[remarkGfm]}>{workData.work.description}</ReactMarkdown>
              </article>

              {workData.work.techStack.length && workData.work.userActions.length && (
                <div className="grid grid-cols-1 sm:grid-cols-3 mb-10 gap-2 text-gray-300 text-[1.4rem] tech-func">
                  <div className="mb-6 sm:mb-0">
                    <h1 className="mb-4 text-2xl font-semibold text-gray-200 capitalize">tech-stack</h1>
                  ...
                  <div className="col-span-2">
                    <h1 className="mb-4 text-2xl font-semibold text-gray-200 capitalize">user-actions</h1>
                    <ul className="flex flex-col gap-y-2">
              ...
              {workData.work.images.slice(1).map((img, idx) => (
                <div
                  key={idx}
                  className={`w-full relative h-[52rem] ${idx !== workData.work.images.length - 2 && "mb-8"}`}
                >
                  <Image src={img.url} alt="project description" layout="fill" objectFit="cover" />
                </div>
              ))}
            </section>
          </div>
